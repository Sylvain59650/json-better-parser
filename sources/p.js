...
const __internal = {
  ...
  isLetter: char => char.match(/[A-Za-z]/),
  isNumber: char => char.match(/[0-9]/),
  isWhitespace: char => char.match(/\s/),
  isNewline: char => char.match(/\n/),

  lex: function({ state, head, char, insert, remove, removePreviousComma, replace, push, pop }) {
    switch (state) {
      case "start":
        if (__internal.isWhitespace(char)) {
          remove();
        }
        if (char === "{") {
          push("obj");
      ...
      case "obj_awaiting_key":
        ...
        if (__internal.isLetter(char) || __internal.isNumber(char) || char === "-") {
          insert("\"");
      ...
      case "obj_awaiting_value":
        ...
        if (__internal.isNumber(char)) { return "obj_value_num"; }
      ...
      case "arr_stable":
        ...
        if (__internal.isNumber(char)) { return "arr_value_num"; }
...
var obj = parse("{a:1,b:2,c:[{cc:{ccc:6},ddd:4,dd:5,}],}");
